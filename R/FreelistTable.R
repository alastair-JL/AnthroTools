...
#' @param tableType Currently there are four types of tables: ÄúPRESENCE, úSUM_SALIENCE,MAX_SALIE, HIGHEST_RANKNCE and FREQUENCY. PRESENCEÄù will give a "1" if a participant mentioned the specified code, or Äú0Äù otherwise. If you specify ÄúFREQUENCYù, then you will get a count of how often each code was mentioned by each person. If you use ÄúSUM_SALIENCEÄù then you will get the total salience each person has associated with each code. If you use ÄúMAX_SALIENCEÄù then you will get the maximum salience, i.e., the salience of the code the first ti me it was mentioned. HIGHEST_RANK gives the lowest value in the order column (the highest rank).
#' @keywords FreeList
